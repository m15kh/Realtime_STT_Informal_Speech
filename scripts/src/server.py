...
from ai_service.models.fish_speech.voice import VoiceService
# --- Configuration ---
...
TTS_COQUI_MODELS_PATH = '../../checkpoints/xtts' # Or path to your local Coqui TTS models directory
TTS_LANGUAGE = "en"
...
async def lifespan(app: FastAPI):
    ...
    # 6. Initialize VoiceService for FishSpeech #FIXME
    cprint("[LOG] Initializing VoiceService for FishSpeech...", 'green')
    try:
        global vs
        vs = VoiceService()
        cprint("[LOG] VoiceService initialized successfully.", 'green')
    except Exception as e:
        cprint(f"[LOG] Failed to initialize VoiceService: {e}", 'red')
        vs = None
...
async def process_llm_and_tts_request(websocket: WebSocket, user_input_content: str, chat_history: list):
    ...
    if tts_stream and tts_engine and full_llm_response_text: 
        ...
        try:
            ...
            audio_tensor = vs.fishspeech(text=full_llm_response_text, 
                      clone_npy_path="/root/m15kh/ai_service/outputs/clone_voice_feature.npy")
            cprint(full_llm_response_text)
            cprint('------', 'yellow')
            cprint(audio_tensor,'yellow')
            cprint("-----------------", 'yellow')
    
            # Send audio bytes through WebSocket
            if websocket.client_state == WebSocketState.CONNECTED:
                if isinstance(audio_tensor, torch.Tensor):
                    # Move to CPU, convert to float32
                    audio_numpy = audio_tensor.cpu().detach().float().numpy()[0, 0]  # Get first channel
                    
                    # Check the original sample rate and resample if needed
                    original_sr = 44100  # FishSpeech's sample rate
                    target_sr = TTS_TARGET_SAMPLE_RATE  # 24000, as set in config
                    
                    if original_sr != target_sr:
                        # Resample the audio - using a simple method for speed
                        import scipy.signal as signal
                        num_samples = int(len(audio_numpy) * target_sr / original_sr)
                        audio_resampled = signal.resample(audio_numpy, num_samples)
                    ...
                    # Send explicit sample rate information
                    await websocket.send_text(f"[AUDIO_INFO] sample_rate={target_sr}")
                    await websocket.send_bytes(audio_bytes)
                elif isinstance(audio_tensor, (bytes, bytearray)):
                    await websocket.send_bytes(audio_tensor)
                else:
                    print(f"Warning: Unexpected audio data type: {type(audio_tensor)}")
                    await websocket.send_text("[ERROR] Invalid audio data format")
